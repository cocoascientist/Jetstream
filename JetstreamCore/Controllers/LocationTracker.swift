...
final class LocationTracker {
    ...
    var locationUpdateEvent: AnyPublisher<Result<Location, Error>, Never> {
        return locationSubject
            .removeDuplicates(by: { (lhs, rhs) -> Bool in
                switch (lhs, rhs) {
                case (.success(let lhsLoc), .success(let rhsLoc)):
    ...
    private func updateLocationSubject(with result: Result<[CLLocation], Error>) {
        ...
        case .failure(let error):
            if locationServicesUnavialable(error) {
                locationSubject.send(.failure(LocationError.unavailable))
            } else {
    ...
    private func locationServicesUnavialable(_ error: Error) -> Bool {
